...
def WriterErrToCSV(msg):
	f = open('../results/fault.csv','a')
	writer = csv.writer(f)
	writer.writerow([msg])
	f.close()
	return
...
def main():
	...
	# ------------- loading solution ------------
	solution = {}
	csvfile = file('../results/test_out.csv','rU') # please input your solution path
	lines = csv.reader(csvfile)
	for line in lines:
	...
	if len(dmen)>C_man:
		...
		WriterErrToCSV('error: the maximum number of postman is out!')
	for dInd in dmen:
		rNum = len(solution[dInd])
		if rNum%2 != 0:
			print 'error: pickup need to match delivery!'
			WriterErrToCSV('error: pickup need to match delivery!')
			# break
		else:
			pickup_nodes = [] 
			delivery_nodes = []
			...
			tmp_load = 0
			tmp_dist_in = 720
			# print 'length is ' + str(len(solution))
			# print solution[dInd]
			# print dInd
			# return
			for rInd in solution[dInd]:
				...
				if tmp_load>C_load:
					...
					WriterErrToCSV('error: the maximum number of packages is out!')
				if rInd[1]-tmp_time<0 or rInd[2]<rInd[1]:
					# print 'the error is caused by '
					# print rInd
					# print rInd[1], tmp_time
					# print rInd[2], rInd[1]
					print 'error: for one postman, the solution must be sorted by arrival time!'
					WriterErrToCSV('error: for one postman, the solution must be sorted by arrival time!')
					# break
				else:
					tmp_time = rInd[2]
				...
				if rInd[3]>0:
					...
					if len(pickup_nodes)!=len(set(pickup_nodes)) or rInd[3]!=orders[rInd[4]][4]:
						...
						WriterErrToCSV('error: one order must be picked one time!')
						# break
					if rInd[0]!=orders[rInd[4]][1]:
						print 'error: the pickup point of order not match original data!'
						WriterErrToCSV('error: the pickup point of order not match original data!')
						# break
				elif rInd[3]<0:
					...
					if -rInd[3]!=orders[rInd[4]][4] or len(delivery_nodes)!=len(set(delivery_nodes)):
						...
						WriterErrToCSV('error: one order must be deliveried one time!')
						# break
					if rInd[0]!=orders[rInd[4]][0]:
						print 'error: the delivery point of order not match original data!'
						WriterErrToCSV('error: the delivery point of order not match original data!')
						# break
					if len(set(delivery_nodes)-set(pickup_nodes))>0:
						print 'error: one order must be picked first, then delivery!'
						WriterErrToCSV('error: one order must be picked first, then delivery!')
				else:
					print 'error: the number of packages is zero!'
					WriterErrToCSV('error: the number of packages is zero!')
					# break
				if rInd[2]<orders[rInd[4]][2]:
					...
					# print orders[rInd[4]]
					# print rInd[2]
					# print rInd[2], orders[rInd[4]][2]
					print 'error: departure time is earlier than the reqired earliest time!'
					WriterErrToCSV('error: departure time is earlier than the reqired earliest time!')
					# break
				...
				if penalty_serv>0 or penalty_rout>0:
					...
					if tmp_cheat>C_cheat:
						...
						WriterErrToCSV('error: do not cheat!')
			...
			# counti = copy.copy(counti) + 1
			# if counti > 0	:
			# 	return

	if len(set(nodeset_check))!=len(set(nodeset_org)):
		print len(set(nodeset_check)), len(set(nodeset_org))
		print 'error: the number of orders not match original data!'
		WriterErrToCSV('error: the number of orders not match original data!')
